...
$.widget('javobyte.rouletteWheel', {
    ...
    _draw: function () {
        ...
        var shadowColor = this.is_rotating() ? '#0090E1' : '#0090E1';
...
//            ctx.arc(cx, cy, innerRadius, angle + arc, angle, true); ////////lines between elements
            ...
            var img = document.getElementById("arrow");  /////////////arrow
            ctx.drawImage(img, -65, -15);
        ...
        if (!this.is_rotating()) {
            ...
            ctx.fillStyle = "white";
            ctx.font = '15px Helvetica, Arial';
            var text = "Spin the wheel";
            ctx.fillText(text, cx - ctx.measureText(text).width / 2, cy + 45);
            ctx.restore();
            /*custom*/
        }
